    ...
    SELECT
      ...
      (
        ...
        + CASE WHEN array_length(words, 1) IS NOT NULL AND p.name IS NOT NULL AND NOT EXISTS (
               ...
               THEN 400 ELSE 0 END
        + (300 * greatest(
            extensions.word_similarity(lower(q), lower(coalesce(p.name, ''))),
            extensions.similarity(lower(coalesce(p.name, '')), lower(q)),
            extensions.similarity(lower(coalesce(p.code, '')), lower(q))))::int
      ) AS score
    ...
    WHERE p.store_id = st.id
      ...
      AND (
        ...
        OR (p.name IS NOT NULL AND array_length(words, 1) IS NOT NULL AND NOT EXISTS (
              SELECT 1 FROM unnest(words) w
              WHERE public.kawscan_norm(p.name) NOT LIKE '%' || public.kawscan_norm(w) || '%'))
        OR (length(qn) >= 3 AND p.name IS NOT NULL
            AND extensions.word_similarity(lower(q), lower(p.name)) > 0.4)
        OR (length(qn) >= 3 AND (
              extensions.similarity(lower(coalesce(p.name, '')), lower(q)) > 0.28
              OR extensions.similarity(lower(coalesce(p.code, '')), lower(q)) > 0.35))
      )
