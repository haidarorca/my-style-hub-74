...
BEGIN
  ...
  t_ids AS MATERIALIZED (SELECT DISTINCT unnest(ids) AS id),
  k_ids AS MATERIALIZED (SELECT DISTINCT unnest(code_ids) AS id),
  ...
  tw AS MATERIALIZED (SELECT DISTINCT tk.tid, w AS word FROM tk, unnest(tk.words) w),
  hit AS MATERIALIZED (
    SELECT DISTINCT c.product_id, tw.tid
    FROM cand c CROSS JOIN LATERAL unnest(c.toks) ct(w) JOIN tw ON tw.word = ct.w
    WHERE c.is_txt
  ),
  raw_sc AS (
