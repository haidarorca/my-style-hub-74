...
export function PaymentForm({ order, onPayment }: Props) {
    ...
    <div className="border-t pt-3 mt-3">
      ...
      <div className="grid grid-cols-2 gap-2">
        <div>
          ...
          <Input type="number" className="h-8 text-sm" value={amount} onChange={e => setAmount(e.target.value)} placeholder={`Max: ${order.balance}`} />
        </div>
        <div>
      ...
      <Button size="sm" className="w-full mt-2 h-8" disabled={!amount || parseFloat(amount) <= 0} onClick={handleSubmit}>
        <Check className="h-3.5 w-3.5 mr-1" /> Enregistrer le paiement
      </Button>
    </div>
  );
}
