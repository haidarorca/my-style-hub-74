            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {rows.map((p) => (
                  <tr
                    key={p.id}
                    className={`border-t transition-colors hover:bg-muted/30 ${selectedProducts.includes(p.id) ? "bg-muted/50" : ""}`}
                  >
                    <td className="p-3">
                      <Checkbox
                        checked={selectedProducts.includes(p.id)}
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              <span className="font-semibold text-foreground">{deleteTarget?.name}</span> sera
      ...
      <AlertDialog open={isBulkDeleteOpen} onOpenChange={setIsBulkDeleteOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Voulez-vous vraiment supprimer les produits sélectionnés ? Cette action est
              irréversible.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-1 flex flex-wrap items-center gap-1 text-[11px] text-muted-foreground">
            <span>Stock {row.stock_total}</span>·<span>{row.variant_count} variante(s)</span>·
