...
async function hydrateGuestLines(lines: GuestCartLine[]) {
  ...
  const [{ data: products }, { data: variants }] = await Promise.all([
    supabase
      .from("products")
      .select(
        `id, name, name_i18n, code, price, vendor_id, product_images(url), profiles:vendor_id(full_name, shop_name, shop_whatsapp, vendor_mode)`,
      )
      .in("id", productIds),
    variantIds.length
      ? supabase
        ...
        const { data, error } = await supabase
          ...
          .select(
            `id, quantity, variant_id, product_id, customization, created_at,
             products!inner(id, name, name_i18n, code, price, vendor_id, product_images(url), profiles:vendor_id(full_name, shop_name, shop_whatsapp, vendor_mode)),
