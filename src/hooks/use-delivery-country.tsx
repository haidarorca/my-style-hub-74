...
export function DeliveryCountryProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (stored) setCountryIdState(stored);
    } catch {
      /* ignore */
    }
    setReady(true);
  ...
  // 2) fallback to user's default address country if nothing chosen
  useEffect(() => {
    if (!ready || countryId || !user) return;
    let cancelled = false;
    (async () => {
  ...
  }, [ready, countryId, user]);

  // 3) fallback to first enabled country
  useEffect(() => {
    if (!ready || countryId || !countries || countries.length === 0) return;
    setCountryIdState(countries[0].id);
  }, [ready, countryId, countries]);
