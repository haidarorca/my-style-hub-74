...
/* ═══════════════════════════════════════════════════════════════
   ...
   Features:
   - Numero fixe KZ-000001 + ref technique ORD-xxx
   - Workflow IMPORT : barre de progression 10 etapes
   - Annulation avec verification (checkCanCancel)
   - Fermeture securisee (confirmation si modifications)
   - Stats paiements + edit/delete
   - Timeline complete
   ═══════════════════════════════════════════════════════════════ */
...
export function OrderDrawer({ order, orderIndex, payments, audit, onClose, onPayment, onEditPayment, onDeletePayment, onWeightRecorded, onStatusChange }: Props) {
  ...
  /* ── Actions avec reset unsaved ── */
  const handlePaymentWrapped = (orderId: string, amount: number, method: string, reference: string, admin: string) => {
    hasUnsavedChanges.current = false;
    onPayment(orderId, amount, method, reference, admin);
  };

  const handleStatusWrapped = (orderId: string, status: string, admin: string) => {
    hasUnsavedChanges.current = false;
    onStatusChange(orderId, status, admin);
  };

  return (
    <>
      <Sheet open={!!order} onOpenChange={(o) => !o && handleCloseRequest()}>
        <SheetContent className="w-full sm:max-w-lg overflow-y-auto p-0">
          <div className="p-4 space-y-4">
              ...
              <div className="space-y-1">
                ...
                <div className="flex gap-2 pt-1 flex-wrap">
                  ...
                  <Badge variant="outline" className={`text-[10px] ${STATUS_COLORS[status] ?? ""}`}>
                    {isImport && order.logistics_status
            ...
            {isImport && (
              <div className="bg-indigo-50 rounded-lg p-3 space-y-2">
                <h3 className="text-xs font-semibold text-indigo-800 flex items-center gap-1.5">
                  <Truck className="h-3.5 w-3.5" /> Workflow IMPORT — Etape {importStep + 1}/{importTotalSteps}
                </h3>
                ...
                <div className="flex gap-1 flex-wrap">
                  {IMPORT_WORKFLOW_STEPS.map((step, i) => (
                    <div key={step.key} className={`text-[9px] px-1.5 py-0.5 rounded-full ${i <= importStep ? (i === importStep ? "bg-indigo-600 text-white font-bold" : "bg-indigo-200 text-indigo-800") : "bg-gray-200 text-gray-400"}`} title={step.label}>
                      {i + 1}
                    </div>
                  ))}
                </div>
                <div className="text-[10px] text-indigo-600 font-medium">
                  {IMPORT_WORKFLOW_STEPS[importStep]?.label ?? ""}
                </div>
              </div>
            )}
            ...
            {/* ═══ HISTORIQUE DES PAIEMENTS avec edit/delete ═══ */}
            <div className="bg-gray-50 rounded-lg p-3 space-y-2">
            ...
            {remaining > 0 && status !== "cancelled" && (
              <div onClick={markUnsaved}>
                <PaymentForm balance={remaining} orderId={order.order_id ?? ""} adminName={adminName} onPayment={handlePaymentWrapped} />
              </div>
            )}
            ...
            <div className="space-y-2 pt-2 pb-4">
              {status === "new" && (
                <>
                  <Button size="sm" className="w-full h-11 bg-emerald-600 hover:bg-emerald-700" onClick={() => handleStatusWrapped(order.order_id ?? "", "confirmed", adminName)}>
              ...
              {isImport && status === "confirmed" && (
                <>
                  <Button size="sm" className="w-full h-11 bg-cyan-600 hover:bg-cyan-700" onClick={() => handleStatusWrapped(order.order_id ?? "", "ordered_from_supplier", adminName)}>
              ...
              {isImport && order.logistics_status === "ordered_from_supplier" && (
                <Button size="sm" className="w-full h-11 bg-teal-600 hover:bg-teal-700" onClick={() => handleStatusWrapped(order.order_id ?? "", "received_at_agent", adminName)}>
                  <Package className="h-4 w-4 mr-1.5" /> Marquer recue chez l agent
                </Button>
              )}
              {isImport && order.logistics_status === "received_at_agent" && (
                <Button size="sm" className="w-full h-11 bg-orange-600 hover:bg-orange-700" onClick={() => handleStatusWrapped(order.order_id ?? "", "awaiting_weighing", adminName)}>
                  <ShieldAlert className="h-4 w-4 mr-1.5" /> A peser
                </Button>
              )}
              {status === "ready" && (
                <Button size="sm" className="w-full h-11 bg-indigo-600 hover:bg-indigo-700" onClick={() => handleStatusWrapped(order.order_id ?? "", "shipped", adminName)}>
                  <Truck className="h-4 w-4 mr-1.5" /> Expedier
                </Button>
              )}
              {status === "shipped" && (
                <Button size="sm" className="w-full h-11 bg-emerald-600 hover:bg-emerald-700" onClick={() => handleStatusWrapped(order.order_id ?? "", "delivered", adminName)}>
        ...
        <div className="fixed inset-0 z-[100] bg-black/50 flex items-center justify-center p-4" onClick={() => setShowCancelDialog(false)}>
          <div className="bg-white rounded-xl p-5 max-w-sm w-full space-y-4" onClick={e => e.stopPropagation()}>
            <h3 className="text-lg font-bold flex items-center gap-2">
              <AlertTriangle className="h-5 w-5 text-red-600" /> Annuler {kzNumber}
            </h3>
            ...
            {!cancelCheck.canCancel ? (
              ...
              <>
                  ...
                  <div className="space-y-1">
                    {cancelCheck.warnings.map((w, i) => (
                      <div key={i} className="flex items-start gap-1.5 text-xs text-amber-700 bg-amber-50 rounded p-2">
                        <Info className="h-3.5 w-3.5 shrink-0 mt-0.5" />{w}
                      </div>
                    ))}
                  </div>
                )}
                ...
                <div>
                  ...
                  <Input placeholder="Ex: Client a annule, produit indisponible..." value={cancelReason} onChange={e => setCancelReason(e.target.value)} className="text-sm h-9" />
                </div>
                {cancelCheck.refundOptions.length > 1 && (
                  <div>
                    ...
                    <Select value={cancelRefundType} onValueChange={setCancelRefundType}>
                      ...
                      <SelectContent>
                        {cancelCheck.refundOptions.map(opt => <SelectItem key={opt} value={opt}>{REFUND_LABELS[opt]}</SelectItem>)}
                      </SelectContent>
                    </Select>
                  </div>
                )}
            ...
            <div className="flex gap-2 pt-2">
              <Button variant="outline" className="flex-1 h-10" onClick={() => setShowCancelDialog(false)}>Retour</Button>
              {cancelCheck.canCancel && (
                <Button variant="destructive" className="flex-1 h-10" disabled={!cancelReason.trim()} onClick={() => { handleStatusWrapped(order.order_id ?? "", "cancelled", adminName); setShowCancelDialog(false); setCancelReason(""); setCancelRefundType(""); }}>
                  <Ban className="h-4 w-4 mr-1" /> Confirmer
                </Button>
              )}
            </div>
          </div>
      ...
      {/* ═══ DIALOGUE FERMETURE SECURISEE ═══ */}
      {showCloseConfirm && (
        <div className="fixed inset-0 z-[110] bg-black/50 flex items-center justify-center p-4" onClick={() => setShowCloseConfirm(false)}>
          <div className="bg-white rounded-xl p-5 max-w-sm w-full space-y-4" onClick={e => e.stopPropagation()}>
            <h3 className="text-lg font-bold flex items-center gap-2">
              <AlertTriangle className="h-5 w-5 text-amber-600" /> Modifications non enregistrees
            </h3>
            <p className="text-sm text-gray-600">
              Vous avez des modifications non enregistrees. Si vous quittez maintenant, ces modifications seront perdues.
            </p>
            <div className="flex gap-2 pt-2">
              <Button variant="outline" className="flex-1 h-10" onClick={() => setShowCloseConfirm(false)}>
                Continuer l edition
              </Button>
              <Button variant="destructive" className="flex-1 h-10" onClick={confirmClose}>
