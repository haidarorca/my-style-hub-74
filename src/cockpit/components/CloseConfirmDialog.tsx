...
export function CloseConfirmDialog({ open, onStay, onLeave }: Props) {
  if (!open) return null;
  return (
    <div className="fixed inset-0 z-[210] flex items-center justify-center" style={{ isolation: "isolate" }}>
      ...
      <div className="relative bg-white rounded-xl shadow-2xl mx-4 w-full max-w-sm p-5 space-y-4">
        <h3 className="text-lg font-bold flex items-center gap-2"><AlertTriangle className="h-5 w-5 text-amber-600" />Modifications non enregistrées</h3>
