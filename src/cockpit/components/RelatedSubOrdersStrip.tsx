...
// sœurs d'une même commande mère. Phase 2.
// ═══════════════════════════════════════════════════════════════
...
export function RelatedSubOrdersStrip({ siblings, currentVendorId, onSelect }: Props) {
  ...
  return (
    <div className="bg-indigo-50 border border-indigo-200 rounded-lg p-2.5 space-y-1.5">
      <div className="flex items-center gap-1.5 text-[10px] font-bold text-indigo-900 uppercase tracking-wide">
        ...
        Commandes liées ({siblings.length} boutiques de la même commande)
      </div>
      <div className="flex gap-1.5 flex-wrap">
        {siblings.map(s => {
          const active = s.vendor_id === currentVendorId;
          return (
            <button
              key={s.vendor_id}
              onClick={() => !active && onSelect(s.vendor_id)}
              disabled={active}
