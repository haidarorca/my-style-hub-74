...
function vendorLabel(isAdmin: boolean): { title: string; subtitle: string; icon: typeof Store; color: string; bg: string } {
  if (isAdmin) {
    return { title: "Boutique Officielle", subtitle: "Kawzone — Garantie & Qualité", icon: ShieldCheck, color: "text-purple-700", bg: "bg-purple-50 border-purple-200" };
  }
  return { title: "Vendeur Partenaire", subtitle: "Produit externe — Import", icon: Users, color: "text-blue-700", bg: "bg-blue-50 border-blue-200" };
}
...
export function OrderItemsPanel({ orderId, onClose }: Props) {
  ...
  useEffect(() => {
    if (!orderId) { setError("Aucune commande sélectionnée."); setLoading(false); return; }
    setLoading(true); setError(""); setEmptyMsg("");
    getOrderItems({ data: { order_id: orderId } })
      .then((result: any) => {
        if (result?.error) setError("Erreur : " + result.error);
        else if (!result || !result.items || result.items.length === 0) {
          setEmptyMsg("Cette commande ne contient aucun article.");
          setData({ items: [], order_total: 0, vendor_summary: [] });
        } else setData(result as OrderItemsResult);
      })
      .catch((err) => setError("Erreur : " + (err?.message || "inconnue")))
          ...
          <div className={`rounded-lg border p-3 ${vLabel.bg}`}>
            ...
            {detailItem.shop_name && (
              <div className="mt-2 flex items-center gap-1 text-xs text-gray-600">
                ...
                <span className="font-medium">{detailItem.shop_name}</span>
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-black/50 flex items-end sm:items-center justify-center" onClick={onClose}>
      <div className="bg-white w-full sm:max-w-md sm:rounded-xl rounded-t-xl max-h-[85vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="overflow-y-auto flex-1 p-4 space-y-4">
          {loading && <div className="flex items-center justify-center py-12 gap-2 text-gray-500"><Loader2 className="h-5 w-5 animate-spin" /><span className="text-sm">Chargement...</span></div>}
          ...
          {emptyMsg && <div className="flex items-center gap-2 text-amber-600 bg-amber-50 rounded-lg p-3 text-sm"><AlertCircle className="h-4 w-4 shrink-0" />{emptyMsg}</div>}
          ...
          {data && (
            <>
              ...
              {data.vendor_summary.length > 0 && (
                <div className="space-y-2">
                  <h4 className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Source</h4>
              ...
              {/* Articles */}
              <div className="space-y-2">
                <h4 className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Articles</h4>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {item.shop_name && (
                        <div className="flex items-center gap-1 mt-1">
                          ...
                          <span className={`text-[10px] font-medium ${item.is_admin_shop ? "text-purple-600" : "text-blue-600"}`}>{item.is_admin_shop ? "Officielle" : "Partenaire"}</span>
